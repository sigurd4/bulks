    ...
    E: Length<Elem = ()> + ?Sized
{
    ...
    where
        N: LengthValue
    {
        if length::value::ge(n, self.start()) && length::value::le(n, self.end())
        {
    ...
    L: LengthValue
{
        ...
        Self: Sized
    {
        ...
        (Range { start, end: mid }, RangeInclusive { start: mid, end })
    }
}
...
mod test
{
    ...
    fn it_works()
    {
        for (i, a) in crate::range_inclusive([(); 1], [(); 8]).enumerate().rev()
        {
            assert!(a <= 8);
            assert_eq!(i + 1, a)
        }
