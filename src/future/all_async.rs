...
impl<B, F, Y> Future for AllAsync<B, F>
where
    B: BufferableBulk,
    F: FnMut(B::Item) -> Y,
    Y: Future<Output = bool>
{
    ...
    fn poll(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Self::Output>
    {
        ...
        future.poll(cx).map(|y| y.is_continue())
    }
}
    ...
    Y: Future<Output = bool>
{
    ...
    {
        let future = unsafe { self.map_unchecked_mut(|x| &mut x.future) };
        future.poll(cx).map(|x| if x { ControlFlow::Continue(()) } else { ControlFlow::Break(()) })
    }
}
...
mod test
{
    ...
    fn it_works()
    {
        ...
        tokio::runtime::Builder::new_multi_thread().enable_all().build().unwrap().block_on(async {
            let y = a.into_bulk().all_async(async |x| x > 0).await;
            assert!(y)
        })
    }
}
